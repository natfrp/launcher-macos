...
class SocketClient {
    ...
    var queue = DispatchQueue(label: "Socket Queue")
    var mainConnection, pushConnection: Socket
    
    init(_ path: String, bufferSize: Int = 1048576) {
        self.path = path
    ...
    private func readMessage<T: SwiftProtobuf.Message>(_ connection: Socket, type: T.Type) throws -> T {
        let size = try! readBytes(mainConnection, count: 4)
